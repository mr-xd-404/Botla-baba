...
module.exports.run = async({api, event, args}) => {
	...
	if (args[0]) {
  api.sendMessage(`Bombing Start...`, event.threadID, (err, info) => setTimeout(() => { api.unsendMessage(info.messageID) }, 90000));
	...
	return api.sendMessage(`- ✅Successfully Bombing Complete🌼\n`, event.threadID, event.messageID)
} //modifi credit - fixed by TANVIR-TAMIM
else if (args.join() == "") { 
	  return api.sendMessage(`🌼••Sms Boomber By Bappa Bot••🌼\n\nএসএমএস বোম্বিং করতে /sms লিখে নাম্বার লেখো🥰(with out +88 ) \n\nপ্রতিবারে সর্বোচ্চ ৫ টি মেসেজ যাবে`, event.threadID, event.messageID)}
}